...
export default function (app, router) {
  ...
  router.get('*', function *(next) { // eslint-disable-line complexity
    ...
    let location = history.createLocation(this.url);
    ...
    try {
      [ redirectLocation, renderProps ] = yield match.bind(this, { routes, history, location });
    } catch (err) {
    ...
    let createMainStore = require(config.react.store).default;
    let reduxStore = createMainStore(history);
