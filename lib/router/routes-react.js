...
export default function (router) {
  ...
  router.get(config.router.react.path, function *(next) { // eslint-disable-line complexity
    ...
    // NPM ReactDom is up to 30% slower than the minified browser version
    // so use the latter on production (see https://github.com/facebook/react/issues/812)
    let ReactDOM = require(isProduction ? 'react/dist/react.min' : 'react-dom/server');
